...
import type { McpTool } from '@agent-harness/mcp';
...
const task: Task = {
  ...
  repoContext: {
    rootPath: process.cwd(),  // analyze THIS repo
...
// Route the task — this analyzes the repo and scores agents
...
console.log(`   workflow/discover → ${(workflows as unknown[]).length} workflow(s) found`);

const status = await handler.handle('workflow/status', { workflowId: machine.current.id });
console.log(`   workflow/status → phase: ${(status as { phase: string })?.phase}`);
